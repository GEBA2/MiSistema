...
<?php
 ...
 $estandar= mysql_query( "SELECT * FROM registrovotante where dui = '".$_REQUEST['var1']."'" ,$conexion);

if(mysql_query( $sql = "UPDATE voto SET voto = voto+1 WHERE `voto`.`id`;")){

    mysql_query("UPDATE registrovotante SET estado=1 WHERE dui='".$_REQUEST['var1']."'");
		}else{
			echo "error";
		}
